...
def upload_video(client):
    video_path = Path(__file__).parent / "videos" / "no_audio.mp4"
...
def test_gait_analysis(settings):
    start_time = 3
    end_time = 6
    chosen_subject_id = 1
    height = 170
    ...
    assert task_frames, "No bounding boxes found in selected gait time range"
    ...
    gait_response = client.post(f"/api/gait/?id={video_id}", {"json_data": json.dumps(json_data)}, format="multipart")
    assert gait_response.status_code == 200
