...
def upload_video(client):
    video_path = Path(__file__).parent / "videos" / "leg_agility_left.mp4"
...
def test_leg_agility_left_analysis(settings):
    start_time = 3
    end_time = 6
    chosen_subject_id = 1
    client = APIClient()
    ...
    assert task_frames, "No bounding boxes found in selected leg_agility_left time range"
    ...
    leg_agility_left_response = client.post(f"/api/leg_agility_left/?id={video_id}", {"json_data": json.dumps(json_data)}, format="multipart")
    assert leg_agility_left_response.status_code == 200
