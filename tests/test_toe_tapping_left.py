...
def upload_video(client):
    video_path = Path(__file__).parent / "videos" / "toe_tapping_left.mp4"
...
def test_toe_tapping_left_analysis(settings):
    start_time = 3
    end_time = 6
    chosen_subject_id = 1
    client = APIClient()
    ...
    assert task_frames, "No bounding boxes found in selected toe_tapping_left time range"
    ...
    toe_tapping_left_response = client.post(f"/api/toe_tapping_left/?id={video_id}", {"json_data": json.dumps(json_data)}, format="multipart")
    assert toe_tapping_left_response.status_code == 200
